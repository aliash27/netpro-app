...
export default function Reports() {
  ...
  function printReport() {
    ...
    const totD = late.reduce((a,s) => a + calcDebt(s).length * s.monthly_fee, 0)
...
<tbody>
${subs.map((sub,i) => {
  const d = calcDebt(sub), t = d.length * sub.monthly_fee
  return `<tr>
    <td>${i+1}</td>
...
<div class="ft">نيت برو v1.0 — ${new Date().toLocaleString('ar-IQ')}</div>
  ...
  const late  = subs.filter(s => calcDebt(s).length > 0)
  const paid  = subs.filter(s => calcDebt(s).length === 0)
  const totD  = late.reduce((a,s) => a + calcDebt(s).length * s.monthly_fee, 0)
  const totR  = pays.reduce((a,p) => a + p.amount, 0)
          ...
          {!late.length ? (
            ...
            .map((sub, i, arr) => {
              ...
              const maxT = Math.max(...late.map(s =>
                calcDebt(s).length * s.monthly_fee))
